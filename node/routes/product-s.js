...
const getListData = async (req, res) => {
  ...
  if (keyword) {
    // 避免 SQL injection
    where += ` AND (
    sd.\`name\` LIKE ${db.products(`%${keyword}%`)} 
    OR
    sd.\`price\` LIKE ${db.products(`%${keyword}%`)}
    )
    `;
  }
  ...
  const sql = `SELECT COUNT(1) totalRows FROM products ps ${where}`;
  ...
  if (totalRows > 0) {
    ...
    ${where} ORDER BY id DESC LIMIT ${(page - 1) * perPage
      }, ${perPage}`;
